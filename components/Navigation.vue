      ...
      <div v-for="link in links" :key="link.name" class="relative">
        ...
        <button
          :class="{
            ...
            'bg-red border-green': isActive(link.name),
            'border-transparent': !isActive(link.name)
          }"
          @click="navigate(link.to)"
          @mouseover="hoverLink = link.name"
        ...
        >
          <fa
            ...
            :class="{
              ...
              'text-white': isActive(link.name),
              'text-green': !isActive(link.name)
            }"
          ></fa>
        </button>
        ...
        <!-- Hover-Label -->
        <div
          v-if="hoverLink === link.name"
          :class="{
            'absolute left-full top-0 h-12 w-24 md:h-24 md:w-40 flex items-center justify-center rounded-r-xl z-0': true,
            'bg-red/70': isActive(link.name),      // halbtransparentes Rot
            'bg-green/70': !isActive(link.name)    // halbtransparentes Grün
          }"
        >
          <span class="text-white text-xl md:text-2xl font-semibold">{{ link.label }}</span>
        </div>
      </div>
    </div>
  </div>
